...
def heatmap(data, row_labels, col_labels,  ax=None,
    ...
    **kwargs
    ...
    #cbar.ax.yaxis.set_major_formatter(plt.NullFormatter())
    #cbar.ax.yaxis.set_minor_locator(plt.MultipleLocator(5))
...
def heatmap_tr(data, row_labels, col_labels,  ax=None,
            cbar_kw={}, cbarlabel="", **kwargs):
    """
    Create a heatmap from a numpy array and two lists of labels.
    ...
    **kwargs
    ...
    # Create colorbar
    cbar = ax.figure.colorbar(im, ax=ax, fraction = 0.0315)
    #cbar.ax.set_yticks([0,63])
    cbar.ax.set_ylabel(cbarlabel, rotation=-90, va="bottom", fontdict = fontaxes)
    cbar.ax.tick_params(axis='both', which='major', labelsize=7)
    #cbar.ax.yaxis.set_major_formatter(plt.NullFormatter())
    ...
    # We want to show all ticks.
    print data.shape[1] , np.arange(data.shape[1])
    ax.set_xticks(np.arange(data.shape[1]))
    ax.set_yticks(np.arange(data.shape[0]))
    # ... and label them with the respective list entries.
    ax.set_xticklabels(col_labels)
